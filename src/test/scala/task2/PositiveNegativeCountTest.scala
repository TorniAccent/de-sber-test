...
class PositiveNegativeCountTest extends org.scalatest.funsuite.AnyFunSuiteLike {
  ...
  test("corner cases") {
    ...
    assert(PositiveNegativeCount.count(List(0, 0, 0, 0)) == 0)
    assert(PositiveNegativeCount.count(List(0, 0, 0, 0, 1)) == 0)
    assert(PositiveNegativeCount.count(List(0, 0, 0, 0, -1)) == 0)
    assert(PositiveNegativeCount.count(List(0, 0, 0, 0, 1, 0)) == 0)
    assert(PositiveNegativeCount.count(List(0, 0, 0, 0, -1, 0)) == 0)
  }
